    ...
    <nav className="fixed w-full h-16 top-0 bg-background/40 border-b-white/40 px-8 backdrop-blur flex items-center justify-between z-50">
      ...
      {/* <div className="container mx-auto px-4 py-3 flex justify-between items-center"> */}
       ...
       <ul className="hidden md:flex w-full justify-end items-center space-x-4 ">
        <li>
          {" "}
          <Link className='hover:text-green-500 transition' href={"/"}>Home</Link>
        </li>
        <div className="w-0"></div>
        <li>
          {" "}
          <Link className='hover:text-green-500 transition' href={"/project"}>Project</Link>
        </li>
        <div className="w-0"></div>

        <li>
          {" "}
          <Link className='hover:text-green-500 transition' href={"/blog"}>Blog</Link>
        </li>
        <div className="w-0"></div>

        <li>
          {" "}
          <Link className='hover:text-green-500 transition' href={"/about"}>About</Link>
        </li>
        <div className="w-0"></div>

        <Link
          href="/contact"
          className={`${buttonVariants({ variant: "outline" })} hover:bg-green-600 hover:border-green-600 hover:text-white transition-all`}
        >
          Contact Me
        </Link>
        ...
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          <SheetContent>
            ...
            <ul className="space-y-4 mt-4">
              <li><Link href="/" onClick={closeSheet}>Home</Link></li>
              <li><Link href="/about" onClick={closeSheet}>About</Link></li>
              <li><Link href="/blog" onClick={closeSheet}>Blog</Link></li>
              <li><Link href="/contact" onClick={closeSheet}>Contact</Link></li>
            </ul>
          </SheetContent>
        </Sheet>
      </div>
